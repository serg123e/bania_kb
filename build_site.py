#!/usr/bin/env python3
"""Сборка статического SPA-сайта вики bania_kb.
...
Публикуется ТОЛЬКО вики. Поле `url` из frontmatter (ссылки на YouTube) в вывод
НЕ попадает. raw/, info.json, log.md, CLAUDE.md сайтом не используются.
...
SOURCE_STYLE = re.compile(r"^[A-Za-z0-9_-]{11} - ")
...
def slug_of(category, stem):
    """sources → короткий youtube-id; остальные → имя файла (kebab-кириллица)."""
    if category == "sources":
